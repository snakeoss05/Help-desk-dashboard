...
import AdpModel from "@/models/AdpModel";
...
export async function GET(req) {
  ...
  const totalItems = await AdpModel.countDocuments(); // Total number of items in the collection
  ...
  try {
    results.results = await AdpModel.find()
      .limit(limit)
      .skip(startIndex)
      .select("-__v");
...
export async function DELETE(req) {
  ...
  const id = searchParams.get("id");
  try {
    await AdpModel.findOneAndDelete({
      _id: new mongoose.Types.ObjectId(id),
    });
