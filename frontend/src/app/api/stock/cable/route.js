...
import cableModel from "@/models/CableModel";
...
export async function GET(req) {
  ...
  const totalItems = await cableModel.countDocuments(); // Total number of items in the collection
  ...
  try {
    results.results = await cableModel
      .find()
      .limit(limit)
      .skip(startIndex)
      .select("-__v");
...
export async function DELETE(req) {
  ...
  const id = searchParams.get("id");
  try {
    await cableModel.findOneAndDelete({
      _id: new mongoose.Types.ObjectId(id),
    });
