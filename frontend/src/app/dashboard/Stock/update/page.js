...
export default function UpdateStock() {
  ...
  async function determineTableHeaders(data) {
    data.forEach((item) => {
  ...
  async function GetPcTable(category, page, limit) {
    await axios
      .get(`/api/stock/${category}?page=${page}&limit=${limit}`)
      .then((res) => {
        const { results, totalPages } = res.data;
        setTotalPages(totalPages);
        setPctable(results.results);
        determineTableHeaders(results.results);
      })
      .catch((e) => {
        console.log(e);
      });
  ...
  const renderQuantity = (quantity, id, isEditing) => {
    return (
      <div className="flex flex-row justify-around items-center w-full">
        ...
        <i
          className="fa-solid fa-pen"
          onClick={() => handleEditQuantity(id)}></i>
      </div>
    );
  };
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter") {
      GetPcTable(selectedCategory, currentPage, posteperpage);
    }
  };
  const handlePageClick = (page) => {
    setCurrentPage(page);
    GetPcTable(selectedCategory, page, posteperpage);
  };
                  ...
                  <tr key={index}>
                    ...
                    <td className="tabledata relative">
                      ...
                      <button
                        ...
                        <i className="fa-solid fa-circle-chevron-up  text-green-600  text-2xl"></i>
                      </button>
                    </td>
                    <td className="tabledata relative">
                      {" "}
